...
namespace hz
{
	Context::Context(const m::String& name, ContextAttribute attr)
		: m_name(name)
		//	, m_stack(0)
		, m_stackAlloc(attr.stackSize)
		, m_poolAlloc(attr.poolSize)
	{
	...
	bool Context::compile()
	{
		...
		// Update ByteCode generated by current compile pass
		// into the context
		ByteCode* bc = m_compiler.compile(m_error);
		if (bc != NULL)
		{
			return true;
		}
